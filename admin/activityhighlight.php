...
$product_sql_Delete = "DELETE FROM Activityhighlight WHERE NewFlag = 1 AND CreateUserID = '$currentUserID'";
mysql_query("UPDATE Activityhighlight SET State = '0' WHERE EndDate <= NOW() ");
...
$query_activityhighlight = "SELECT DISTINCT A.*,S.UserName,R.CodeName FROM Activityhighlight A ";
$query_activityhighlight .= "LEFT JOIN SystemUsers S ON IFNULL(A.UpdateUserID, A.CreateUserID) = S.UserID ";
$query_activityhighlight .= "LEFT JOIN RefCommon R ON A.Category = R.TypeCode AND R.Type = 'Activityhighlight' WHERE 1 AND A.NewFlag = '0' ";
if (trim($searchText) != "") {
	$query_activityhighlight .= " AND A.ShortTitle like '%".$searchText."%' ";	
}
$query_activityhighlight .= " ORDER BY State, StartDate DESC, EndDate";
...
<head id="MainMasterHead">
    ...
    <script type="text/javascript">
        ...
        function setdetail(intNo){
        	if (intNo ==""){				
        		$("#iDetail").attr("src","activityhighlightDetail.php");	
        	}else{				
        		$("#iDetail").attr("src","activityhighlightDetail.php?activityhighlightID="+intNo);	
        	}
                        ...
                        <div id="divDetailBody" class="divDetailBody" style="padding-top: 0px; border: 0px;">
                            <div id="divGridview">
                                ...
                                <div style="height: 36px; padding-top: 5px; border-left:1px solid #CCCCCC; border-top:1px solid #CCCCCC; border-right:1px solid #CCCCCC;">
                                    <form action="activityhighlight.php" method="POST" enctype="multipart/form-data" id="startSearch" name="startSearch">
                                    ...
                                    <div style="float: right;padding-top: 4px;">
                                        <input type="button" name="ibAdd" id="btnAdd" class="add" value=" 新增活動花絮 " onclick="setdetail('')"/>
                                        &nbsp;&nbsp;
                                    </div>
                                </div>
                                <div>
                                    <table class="GridView" cellspacing="0" cellpadding="5" rules="all" border="1" id="gvGridView"
										...
										<tr>
											...
											<td align="center">
												<input id="btnEdit" name="btnEdit" class="detail"  type="button" value="修改" onclick="setdetail('<?echo $row["ActivityhighlightID"]?>')" />
                                                &nbsp;&nbsp;&nbsp;&nbsp;
							...
							<div class="GridViewFooter" align="center">
                                <table class="TableNoLine">
                                    <tr>
                                        ...
                                        <td>
											<a href="activityhighlight.php?page=1&searchText=<?echo $searchText?>">最前頁｜</a>
                                        </td>
                                        <?	
											$prePage = $page-1;
											if ($prePage < 1) {
										...
										?>
										<td>
											<a href="activityhighlight.php?page=<?echo $prePage?>&searchText=<?echo $searchText?>">上頁｜</a>
                                        </td>
										<?	
											$nextPage = $page+1;
										...
										?>
                                        <td>
											<a href="activityhighlight.php?page=<?echo $nextPage?>&searchText=<?echo $searchText?>">下頁｜</a>
                                        </td>
                                        <td>
											<a href="activityhighlight.php?page=<?echo $total_pages?>&searchText=<?echo $searchText?>">最後頁</a>
                                        </td>
                                        <td>
