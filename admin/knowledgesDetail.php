...
if (isset($_POST["knowledgesID"])) {
	$knowledgesID = $_POST["knowledgesID"];
}
$mode="0";
$modeDesc="修改";
$isNew = false;
if ($knowledgesID == ""){
	$isNew = true;
	$insert_sql = "INSERT INTO Knowledges(NewFlag, CreateUserID) VALUES(1, '$currentUserID')";
	//echo $insert_sql;
	mysql_query($insert_sql);
	$knowledgesID = mysql_insert_id();
	$modeDesc = "新增知識分享";
	$mode = "1";
}else{
	$rec_newFlag = mysql_query("SELECT NewFlag FROM Knowledges WHERE knowledgesID = '$knowledgesID'");
	if($rec_newFlag){
...
if ($_POST["action"] == "update"){ 
	...
	$update .= "NewFlag=0 WHERE KnowledgesID='$knowledgesID'";
	...
	//picture upload
    $targetFolder = "/home/metroasis/public_html/images/knowledges/";
    if (!file_exists($targetFolder)) {
    	@mkdir($targetFolder);
    }
    
    if ($knowledgesID != ""){
    	$targetFolder = "/home/metroasis/public_html/images/knowledges/$knowledgesID";
    	if (!file_exists($targetFolder)) {
    		@mkdir($targetFolder);
    	}
    }
    
    $target_dir = "/home/metroasis/public_html/images/knowledges/$knowledgesID/";
	$ImagefilePath = "/images/knowledges/$knowledgesID/";
	$Message ="";
		...
		if ($src_w==$intWidth && $src_h==$intHight){
			if (move_uploaded_file($_FILES['imgFile1']['tmp_name'], $target_dir . $_FILES['imgFile1']['name'])) {
				...
				if ($ImageID1 != ""){
					...
					$insert = "INSERT INTO ImagesFiles (ForeignID,ImageFunction,ImageType,ImagePath,ImageFileName) VALUES ('$knowledgesID','knowledges','detail','$ImagefilePath','". $_FILES['imgFile1']['name'] ."')";
				}                    
				//echo $insert;
	            mysql_query($insert);
	        }
        } else {
	    	$Message = "「知識分享 圖片」應為 (500 * 333)";
	    	mysql_query("INSERT INTO Knowledges (`Title`, `ShortTitle`, `Content`, `ShortContent`, `StartDate`, `EndDate`, `State`, `Category`, `Image`, `IsTop`, `ReadCount`, `NewFlag`, CreateUserID, CreateDate, UpdateUserID, UpdateDate) SELECT `Title`, `ShortTitle`, `Content`, `ShortContent`, `StartDate`, `EndDate`, `State`, `Category`, `Image`, `IsTop`, `ReadCount`, 1 AS NewFlag, CreateUserID, CreateDate, UpdateUserID, UpdateDate FROM Knowledges WHERE KnowledgesID = '$knowledgesID'");
			$knowledgesId4delete = $knowledgesID;
			$knowledgesID = mysql_insert_id();
			mysql_query("DELETE FROM Knowledges WHERE KnowledgesID = '$knowledgesId4delete'");
	    }
	}
	...
	if ($Message != ""){
		...
		echo "<script>parent.location.href='knowledges.php'; </script>";
	}
}
$query = "SELECT N.*,S.UserName AS createUserName, S2.UserName AS updateUserName, ";
$query .= "(SELECT concat(ImagePath,ImageFileName) as ImageFile FROM ImagesFiles WHERE ImageFunction = 'knowledges' AND ImageType ='detail' AND ForeignID ='$knowledgesID' LIMIT 1) AS ImageFile, ";
$query .= "(SELECT ImageID FROM ImagesFiles WHERE ImageFunction = 'knowledges' AND ImageType ='detail' AND ForeignID ='$knowledgesID' LIMIT 1) AS ImageID ";
$query .= "FROM Knowledges N ";
$query .= "LEFT JOIN SystemUsers S ON N.CreateUserID = S.UserID ";
$query .= "LEFT JOIN SystemUsers S2 ON N.UpdateUserID = S2.UserID ";
$query .= "WHERE N.knowledgesID = '$knowledgesID' ";
//echo $query;
$RecKnowledges = mysql_query($query);
$row=mysql_fetch_assoc($RecKnowledges);
?>
...
<head>
    ...
    <script type="text/javascript" charset="UTF-8">
		...
		function beforeSubmit(){
        	...
        	showLoading();
        	var textFields = ["StartDate","ShortTitle","ShortContent","Title"];
        	var textNames = ["上架日期","列表標題","短文","內頁標題"];
...
<body>
    <form name="form1" method="post" action="knowledgesDetail.php" id="form1" enctype="multipart/form-data">
    <div>
            ...
            <div id="divDetailBody" class="divDetailBody" style="padding-top: 0px;">
                <table class="TableLine" border="1px" cellpadding="5px" width="100%" bordercolor="#BABAD2">
                    <tr>
						...
						<td align="center" bgcolor="#ffffff" style="width: 12%">
							<?
								$findCategory = "SELECT * FROM RefCommon WHERE Type = 'knowledges' AND TypeCode <> 0 ORDER BY SortNo";
                ...
                <div style="width:100%; text-align:center">
       	      <p style="height:20px;"></p>
            		...
            		<input type="hidden" name="knowledgesID" value="<? echo $knowledgesID?>"/>
