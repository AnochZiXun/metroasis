        ...
        <div class="row">
            ...
            <div class="row rightBOX">
                    ...
                    <div class="col-lg-12 col-xs-6 ex01">
                        <a href="product_detail.php<?echo '?ProductID='.$row["ProductID"].'&ProductCategoryID='.$productCategoryID.'&SortType='.$sortType?>">
                            <div class="g01">
                                ...
                                <?php 
									$query_ImagesFIles = "SELECT ImagePath , ImageFileName FROM `ImagesFiles` WHERE ForeignID = '" . $row["ProductID"] . "' AND ImageFunction = 'products' AND ImageType = 'detail' LIMIT 1";
				...
				<?}?>
                    <div class="pageBOX">
                      <ul class="pagination">
                        <li><a href="product_s.php?page=1<?echo '&ProductCategoryID='.$productCategoryID.'&SortType='.$sortType?>"><span aria-hidden="true">&laquo;</span></a></li>
                        <? for ($i=0; $i<$total_pages; $i++) {?>
                          <? if ($_GET["page"] == $i+1) {?>
                            <li class="active"><a href="product_s.php?page=<? echo $i+1?><?echo '&ProductCategoryID='.$productCategoryID.'&SortType='.$sortType?>"><? echo $i+1 ?></a></li>
                          <? } else { ?>
                            <li><a href="product_s.php?page=<? echo $i+1?><?echo '&ProductCategoryID='.$productCategoryID.'&SortType='.$sortType?>"><? echo $i+1 ?></a></li>
                          <? } ?>
                        <? } ?>
                        <li><a href="product_s.php?page=<? echo $total_pages==0?1:0;?><?echo '&ProductCategoryID='.$productCategoryID.'&SortType='.$sortType?>"><span aria-hidden="true">&raquo;</span></a></li>
                      </ul>
                    </div>
                  </div>
            </div>
