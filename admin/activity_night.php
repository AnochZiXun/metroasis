...
mysql_query("UPDATE ActivityNight SET Visibility = '0' WHERE StartDate <= NOW() ");
if (isset($_POST["delete"])) {
    $activityNightID = $_POST["ActivityNightID"];
    $rec_deathNote = mysql_query("SELECT CONCAT('Batch:', Batch, ' / Killer:', '$currentUserID', ' / Time:', NOW() ) AS DeathNote FROM ActivityNight WHERE ActivityNightID = '$activityNightID'");
    $deathNote = "unknown";
    if($rec_deathNote){
        $row_deathNote = mysql_fetch_assoc($rec_deathNote);
        $deathNote = $row_deathNote["DeathNote"];
    }
    mysql_query("UPDATE ActivityNight SET Batch='0' ,ValidFlag = '0', DeathNote = '$deathNote' WHERE ActivityNightID = '$activityNightID'");
    header("Location: activity_night.php");
}
$query_activityNight  = "SELECT A.*,S.UserName,(SELECT CONCAT(ImagePath, ImageFileName) FROM ImagesFiles WHERE ImageFunction = 'activityNight' AND ImageType = 'promotePicture' AND ForeignID = A.ActivityNightID LIMIT 1) AS ImageFile FROM ActivityNight A ";
$query_activityNight .= "LEFT JOIN SystemUsers S ON IFNULL(A.UpdateUserID, A.CreateUserID) = S.UserID WHERE 1 AND A.NewFlag = '0' AND ValidFlag = '1'";
$query_activityNight .= " ORDER BY Status ASC, StartDate DESC, Batch DESC";
//echo $query_activityNight;
$recActivity = mysql_query($query_activityNight);
//預設每頁筆數
$pageRow_records = 10;
//總筆數
...
$recActivity = mysql_query($query_activityNight.' LIMIT '.$start.', '.$pageRow_records);
...
<head>
    ...
    <script type="text/javascript">
        ...
        function setdetail(intNo){
            if (intNo ==""){
                $("#iDetail").attr("src","activityDetail_night.php"); 
            }else{
                $("#iDetail").attr("src","activityDetail_night.php?activityNightID="+intNo);   
            }
            //$('#divWork').animate({scrollTop: $("#divDetail").offset().top - 50}, 'slow');
        }
        function transfer2Detail(activityNightID, $this){
            var page = <? echo $page ?>;
            location.href = "activityDetail_night.php?page=" + page + "&activityNightID=" + activityNightID;
        }
                            ...
                            <div id="divGridview">
                                ...
                                <div style="height: 36px; padding-top: 5px; border-left:0px solid #CCCCCC; border-top:0px solid #CCCCCC; border-right:0px solid #CCCCCC;">
                                    <div style="float: right;padding-top: 4px;">
                                        <input type="button" name="ibAdd" id="btnAdd" class="add" value="新增活動" onclick="javascript:location.href='activityDetail_night.php'"/>
                                        &nbsp;
                                    </div>
                                </div>
                                <div>
                                    <table class="GridView" cellspacing="0" cellpadding="5" rules="all" border="1" id="gvGridView" style="border-collapse: collapse;">
                                        <tr>
                                            ...
                                            <th scope="col" style="width: 8%">
                                                <span class="w_12">營地</span>
                                            </th>
                                            <th scope="col" style="width: 8%">
                                                <span class="w_12">活動地點</span>
                                            </th>
                                            <th scope="col" colspan="3" style="width: 11%; padding:0px">
                                            	<table cellpadding="0" cellspacing="0" width="100%" border="0">
                                                    <tr>
                                                        <th align="center" colspan="3">
                                                      	 <span class="w_12">車輛</span>
                                                        </th>
                                                    </tr>
                                                    <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td align="center">
                                                <span class="b_12">1-<? echo setData($row["Batch"]); ?></span>
                                            </td>
                                            <td align="center">
                                                <? 
                                                    if ($row["Visibility"] == "1"){
                                                ...
                                                <?      
                                                        }else if($row["StartDate"] <= date("Y-m-d") && $row["EndDate"] >= date("Y-m-d")){
                                                ?>
                                                            <img src="images/up_green.png" width="28px" height="28px" />
                                                <?
                                                        }else{
                                            ...
                                            <td align="center" style="padding:0px;">
                                            	<div><span class="b_12"><? echo setData($row["StartDate"]); ?></span></div>
                                                <div><span class="b_10">∣</span></div>
                                                <div style="height:4px;"></div>
                                                <div><span class="b_12"><? echo setData($row["EndDate"]); ?></span></div>
                                            </td>
                                            <td align="center">
                                                <span class="b_12"><? echo setData($row["Campsite"]); ?></span>
                                            </td>
                                            <td align="center">
                                                <span class="b_12"><? echo setData($row["Location"]); ?></span>
                                            </td>     
                                            <td align="center">
                                                <span class="b_12"><? echo setData($row["Quota"]); ?></span>
                                            </td>
                                            <?php
                                            $activityID = $row["ActivityNightID"];
                                            ...
                                            <td align="left" valign="top" style="padding-left:5px;">
                                                <pre class="b_12" style="word-wrap: break-word; white-space: pre-wrap;"><? echo $row["Cost"];?></pre>
                                            </td>      
                                            -->
                                            <td align="center">
                                                <input type="button" value="清單" onclick="javascript:location.href='activityRegister.php?actType=<?php echo $actType?>&activityID=<?php echo $row["ActivityNightID"]?>'"/>
                                            </td>
                                            <td align="center">
                                            ...
                                            <td align="center">    
                                                <input name="btnEdit" id="etnEdit<? echo $start + $rowCount ?>" type="button" value="修改" onclick="transfer2Detail('<?echo $row["ActivityNightID"]?>',this)"/>
                                                <form action="" method="POST" enctype="multipart/form-data" id="delete" name="delete">
                                                    ...
                                                    <input name="ActivityNightID" type="hidden" value="<? echo $row["ActivityNightID"]; ?>"/>
                            ...
                            <div class="GridViewFooter" align="center">
                                <table class="TableNoLine">
                                    <tr>
                                        ...
                                        <td>
                                            <a href="activityNight.php?page=1">最前頁｜</a>
                                        </td>
                                        <?  
                                            $prePage = $page-1;
                                            if ($prePage < 1)
                                        ...
                                        ?>
                                        <td>
                                            <a href="activityNight.php?page=<?echo $prePage?>">上頁｜</a>
                                        </td>
                                        <?  
                                            $nextPage = $page+1;
                                        ...
                                        ?>
                                        <td>
                                            <a href="activityNight.php?page=<?echo $nextPage?>">下頁｜</a>
                                        </td>
                                        <td>
                                            <a href="activityNight.php?page=<?echo $total_pages?>">最後頁</a>
                                        </td>
                                        <td>
