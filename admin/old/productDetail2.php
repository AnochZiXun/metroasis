...
<head id="MainMasterHead">
    ...
    <script type="text/javascript" charset="UTF-8">		
        function pageInitial(){
			//$("#form1 .submit").click(function (event) { if (!$("#form1").validationEngine("validate")) { stopLoading(); return false; } });
            //$("#form1").validationEngine({ promptPosition: "bottomLeft" }); // OPENNING BOX POSITION, IMPLEMENTED: topLeft, topRight, bottomLeft,  centerRight, bottomRight
		...
		function addTableRow(strTable){
		    if (strTable =="tbProductsCategorys"){
    		    ...
    		    if (intExist==0){
    		        var strTemplate = "<tr id='trPC"+rowCount+"'><td>"+strProductCategory1+"</td><td>"+strProductCategory2+"</td><td>"+strProductCategory3+"<input id='ProductCategory3_"+ rowCount +"' name='ProductCategory3[]' type='hidden' value='"+strValue+"'/></td><td style='text-align:center'><input name='btnProductsCategoryDelete' type='button' value='刪除' onClick='delTableRow(this);'/></td></tr>";
		...
		function checkProdutsCategorys(){
                    ...
                    <div id="UpdatePanel1">
                        ...
                        <div id="divDetailBody" class="divDetailBody">
        				    ...
        				    <tr>
                                ...
                                <td>
                                    <input name="ProductNo" type="text" class="TextBox" value="<?echo $row["ProductNo"]?>"/>
                                </td>
                                <td style="width: 10%" align="center">
                                    <span class="Mandatory">＊商品名稱</span>
                                </td>
                                <td colspan="5">
                                    <input name="ProductName" type="text" class="validate[required] TextBox" style="width:95%" value="<?echo $row["ProductName"]?>"/>
                                </td>
                            </tr>
                            <tr>
                                <td style="width: 10%" align="center">
        				    ...
        				    <tr>
                                ...
                                <td colspan="7" >
                                    <table id="tbDelivery" class="TableNoLine" border="0px" cellpadding="5px">
                                        <tr>
                                            <?
        										$chkDeliverExpress = '';
        										$chkTakeFromStore = '';
        										if ($row["DeliverExpress"] == 1) {
        											$chkDeliverExpress = 'checked';
        										} 
        										
        										if ($row["TakeFromStore"] == 1) {
        											$chkTakeFromStore = 'checked';
        										}
        									?>
                                            <td><input name="chkDeliverExpress" type="checkbox" class="checkbox" value="1" style="cursor:pointer" <? echo $chkDeliverExpress?>></td>
                                            <td>貨運宅配</td>
                                            <td><input name="chkTakeFromStore" type="checkbox" class="checkbox" value="1" style="cursor:pointer" <? echo $chkTakeFromStore?>></td>
                                            <td>門市取貨</td>
                                        </tr>
                                    </table>
        					...
        					<tr>
                                ...
                                <td colspan="7" >
                                    <table id="tbImage" class="TableLine" border="1px" cellpadding="5px" width="100%" bordercolor="#BABAD2">
        				                <tr>
        				                    <?  $Image_sql_result = "SELECT concat(ImagePath,ImageFileName) as ImageFile FROM ImagesFiles WHERE ImageFunction = 'products' AND ImageType ='detail' AND ForeignID ='".$row["ProductID"] . "' ORDER BY ImageID";
        				                ...
        				                <tr>
        				                    <td><input type="file" name="imgFile[]" id="imgFile1" accept="image/*" onchange="loadFile(event,'image1')" style="width:70px"><? echo $file1Result?></td>
        				                    <td><input type="file" name="imgFile[]" id="imgFile2" accept="image/*" onchange="loadFile(event,'image2')" style="width:70px"><? echo $file2Result?></td>
        				                    <td><input type="file" name="imgFile[]" id="imgFile3" accept="image/*" onchange="loadFile(event,'image3')" style="width:70px"><? echo $file3Result?></td>
        				                    <td><input type="file" name="imgFile[]" id="imgFile4" accept="image/*" onchange="loadFile(event,'image4')" style="width:70px"><? echo $file4Result?></td>
        				                    <td><input type="file" name="imgFile[]" id="imgFile5" accept="image/*" onchange="loadFile(event,'image5')" style="width:70px"><? echo $file5Result?></td>
        				                    <td><input type="file" name="imgFile[]" id="imgFile6" accept="image/*" onchange="loadFile(event,'image6')" style="width:70px"><? echo $file6Result?></td>
        				                </tr>
        				            </table>
        				            <br/>
                                </td>
