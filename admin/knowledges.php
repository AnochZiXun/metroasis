...
$product_sql_Delete = "DELETE FROM Knowledges WHERE NewFlag = 1 AND CreateUserID = '$currentUserID'";
mysql_query("UPDATE Knowledges SET State = '0' WHERE EndDate <= NOW() ");
mysql_query($product_sql_Delete);
if (isset($_POST["delete"])) {
	$KnowledgesID = $_POST["KnowledgesID"];
	$deleteSql = "delete from Knowledges where KnowledgesID = '$KnowledgesID'";
	mysql_query($deleteSql);
	header("Location: knowledges.php");
}
$searchText = (isset($_POST["searchText"]) && $_POST["searchText"] != "") ? $_POST["searchText"] : (isset($_GET["searchText"]) ? $_GET["searchText"] : "");
$query_knowledges = "SELECT K.*,S.UserName,R.CodeName FROM Knowledges K ";
$query_knowledges .= "LEFT JOIN SystemUsers S ON IFNULL(K.UpdateUserID, K.CreateUserID) = S.UserID ";
$query_knowledges .= "LEFT JOIN RefCommon R ON K.Category = R.TypeCode AND R.Type = 'Knowledges' WHERE 1 AND K.NewFlag = '0' ";							
if (trim($searchText) != "") {
	$query_knowledges .= ' AND K.ShortTitle like "%'.$searchText.'%" ';	
}
$query_knowledges .= " ORDER BY State, StartDate DESC, EndDate";
...
<head id="MainMasterHead">
    ...
    <script type="text/javascript">
        ...
        function setdetail(intNo){
        	if (intNo ==""){
        		$("#iDetail").attr("src","knowledgesDetail.php");	
        	}else{
        		$("#iDetail").attr("src","knowledgesDetail.php?knowledgesID="+intNo);	
        	}
            ...
            <div id="divWork">
                <div class="divWorkArea">
                    <div id="UpdatePanel1">
                        <div id="divDetailBody" class="divDetailBody" style="padding-top: 0px; border: 0px;">
                            <div id="divGridview">
                                ...
                                <div style="height: 36px; padding-top: 5px; border-left:1px solid #CCCCCC; border-top:1px solid #CCCCCC; border-right:1px solid #CCCCCC;">
                                    <form action="knowledges.php" method="POST" enctype="multipart/form-data" id="startSearch" name="startSearch">
                                    ...
                                    <div style="float: right;padding-top: 4px;">
                                        <input type="button" name="ibAdd" id="btnAdd" class="add" value=" 新增知識分享 " onclick="setdetail('')"/>
                                        &nbsp;&nbsp;
                                    </div>
                                </div>
                                <div>
                                    <table class="GridView" cellspacing="0" cellpadding="5" rules="all" border="1" id="gvGridView"
										...
										<tr>
											...
											<td align="center">
												<input id="btnEdit" name="btnEdit" class="detail"  type="button" value="修改" onclick="setdetail('<?echo $row["KnowledgesID"]?>')" />
                                                &nbsp;&nbsp;&nbsp;&nbsp;
                                                <form action="" method="POST" enctype="multipart/form-data" id="delete" name="delete">
													...
													<input name="KnowledgesID" type="hidden" value="<? echo $row["KnowledgesID"]; ?>"/>
							...
							<div class="GridViewFooter" align="center">
                                <table class="TableNoLine">
                                    <tr>
                                        ...
                                        <td>
											<a href="knowledges.php?page=1&searchText=<?echo $searchText?>">最前頁｜</a>
                                        </td>
                                        <?	
											$prePage = $page-1;
											if ($prePage < 1) {
										...
										?>
										<td>
											<a href="knowledges.php?page=<?echo $prePage?>&searchText=<?echo $searchText?>">上頁｜</a>
                                        </td>
										<?	
											$nextPage = $page+1;
										...
										?>
                                        <td>
											<a href="knowledges.php?page=<?echo $nextPage?>&searchText=<?echo $searchText?>">下頁｜</a>
                                        </td>
                                        <td>
											<a href="knowledges.php?page=<?echo $total_pages?>&searchText=<?echo $searchText?>">最後頁</a>
                                        </td>
                                        <td>
                        ...
                        <br/>
                        <div id="divDetail">
	                    	<iframe id="iDetail" src="" width="100%" height="1150" frameborder="0" scrolling="no"></iframe>
	                    </div>
                    </div>
                </div>
            </div>
