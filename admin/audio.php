...
$product_sql_Delete = "DELETE FROM Audio WHERE NewFlag = 1 AND CreateUserID = '$currentUserID'";
mysql_query($product_sql_Delete);
mysql_query("UPDATE Audio SET State = '0' WHERE EndDate <= NOW() ");
if (isset($_POST["delete"])) {
	$audioId = $_POST["audioId"];
	$deleteSql = "delete from Audio where AudioID = '$audioId'";
	//echo $deleteSql;
	mysql_query($deleteSql);
	header("Location: audio.php");
}
$searchText = (isset($_POST["searchText"]) && $_POST["searchText"] != "") ? $_POST["searchText"] : (isset($_GET["searchText"]) ? $_GET["searchText"] : "");
$query_audio = "SELECT N.*,S.UserName,R.CodeName FROM Audio N ";
$query_audio .= "LEFT JOIN SystemUsers S ON IFNULL(N.UpdateUserID, N.CreateUserID) = S.UserID ";
$query_audio .= "LEFT JOIN RefCommon R ON N.Category = R.TypeCode AND R.Type = 'audio' WHERE 1 AND N.NewFlag = '0' ";
if (trim($searchText) != "") {
	$query_audio .= ' AND N.ShortTitle like "%'.$searchText.'%" ';	
}
$query_audio .= " ORDER BY State, StartDate DESC, EndDate";
...
<head id="MainMasterHead">
    ...
    <script type="text/javascript">
        ...
        function setdetail(intNo){
        	if (intNo ==""){
        		$("#iDetail").attr("src","audioDetail.php");	
        	}else{
        		$("#iDetail").attr("src","audioDetail.php?audioId="+intNo);	
        	}
                        ...
                        <div id="divDetailBody" class="divDetailBody" style="padding-top: 0px; border: 0px;">
                            <div id="divGridview">
                               ...
                               <div style="height: 36px; padding-top: 5px; border-left:1px solid #CCCCCC; border-top:1px solid #CCCCCC; border-right:1px solid #CCCCCC;">
                                    <form action="audio.php" method="POST" enctype="multipart/form-data" id="startSearch" name="startSearch">
                                    ...
                                    <div style="float: right;padding-top: 4px;">
                                        <input type="button" name="ibAdd" id="btnAdd" class="add" value=" 新增影音內容 " onclick="setdetail('')"/>
                                        &nbsp;&nbsp;
                                    </div>
                                </div>
                                <div>
                                    <table class="GridView" cellspacing="0" cellpadding="5" rules="all" border="1" id="gvGridView"
										...
										<tr>
											...
											<td align="center">
												<input id="btnEdit" name="btnEdit" class="detail"  type="button" value="修改" onclick="setdetail('<?echo $row["AudioID"]?>')" />
                                                &nbsp;&nbsp;&nbsp;&nbsp;
                                                <form action="" method="POST" enctype="multipart/form-data" id="delete" name="delete">
													...
													<input name="audioId" type="hidden" value="<? echo $row["AudioID"]; ?>"/>
							...
							<div class="GridViewFooter" align="center">
                                <table class="TableNoLine">
                                    <tr>
                                        ...
                                        <td>
											<a href="audio.php?page=1&searchText=<?echo $searchText?>">最前頁｜</a>
                                        </td>
                                        <?	
											$prePage = $page-1;
											if ($prePage < 1) {
										...
										?>
										<td>
											<a href="audio.php?page=<?echo $prePage?>&searchText=<?echo $searchText?>">上頁｜</a>
                                        </td>
										<?	
											$nextPage = $page+1;
										...
										?>
                                        <td>
											<a href="audio.php?page=<?echo $nextPage?>&searchText=<?echo $searchText?>">下頁｜</a>
                                        </td>
                                        <td>
											<a href="audio.php?page=<?echo $total_pages?>&searchText=<?echo $searchText?>">最後頁</a>
                                        </td>
                                        <td>
