...
$Sorting = ' order by BranchId';
$query_branches = $sql.$Sorting;	

//search	
if (isset($_POST["searchText"])) {
	$key = trim($_POST["searchText"]);
	$query_branches = $sql.' and BranchName like "%'.$key.'%"'.$Sorting;	
}

$RecBranches = mysql_query($query_branches);
...
$pageRow_records = 14;
//總筆數
$total_records = mysql_num_rows($RecBranches);
...
//echo $query_branches.' LIMIT '.$start.', '.$pageRow_records;
$RecBranches = mysql_query($query_branches.' LIMIT '.$start.', '.$pageRow_records);
    ...
    <script type="text/javascript" charset="UTF-8">		
        function pageInitial() {
            ...
            $(".detail").colorbox({ iframe: true, width: "100%", height: "100%", overlayClose: false, escKey: false, 
									onClosed: function () { location.href="branchs.php"; } });
            $(".add").colorbox({ iframe: true, width: "100%", height: "100%", overlayClose: false, escKey: false, href: "branchDetail.php", 
									onClosed: function () { location.href="branchs.php"; } });
                            ...
                            <div id="divGridview" style="overflow: auto">
                                <div>
                                    <table class="GridView" cellspacing="0" cellpadding="5" rules="all" border="1" id="gvGridView"
                                        ...
                                        <? while($row=mysql_fetch_assoc($RecBranches)){ ?>
										<tr>
											<td align="center">
                                                <?
													if ($row["District"] == "1") {
											...
											<td align="center">
												<? echo $row["BranchPhoneNo"]; ?>
											</td>											
                                            <td align="center">
                                                <a id="gvGridView_ctl02_hlContents" class="detail" href="branchDetail.php?branchId=<? echo $row["BranchId"]; ?>">
                                                    內容設定
												</a>
                                            </td>
											<td align="center">
                            ...
                            <div class="GridViewFooter">
                                <table class="TableNoLine">
                                    <tr>
                                        ...
                                        <td>
											<a href="branchs.php?page=1">最前頁｜</a>
                                        </td>
                                        <?	
											$prePage = $page-1;
											if ($prePage < 1) {
										...
										?>
										<td>
											<a href="branchs.php?page=<?echo $prePage?>">上頁｜</a>
                                        </td>
										<?	
											$nextPage = $page+1;
										...
										?>
                                        <td>
											<a href="branchs.php?page=<?echo $nextPage?>">下頁｜</a>
                                        </td>
                                        <td>
											<a href="branchs.php?page=<?echo $total_pages?>">最後頁</a>
                                        </td>
                                        <td>
