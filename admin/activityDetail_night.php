...
$page = $_GET["page"];
$isNew = false;
if(isset($_GET["activityNightID"])){
    $activityNightID = $_GET["activityNightID"];
    $rec_newFlag = mysql_query("SELECT NewFlag FROM ActivityNight WHERE ActivityNightID = '$activityNightID'");
    if($rec_newFlag){
    ...
    if($_POST["action"] != "update"){
        mysql_query("DELETE FROM ActivityNight WHERE NewFlag = 1 AND CreateUserID = '$currentUserID'");
        $maxBatch = 1;
        $rec_maxBatch = mysql_query("SELECT MAX(Batch)+1 AS MaxBatch FROM ActivityNight");
        if($rec_maxBatch){
            $row_maxBatch = mysql_fetch_assoc($rec_maxBatch);
            $maxBatch = $row_maxBatch["MaxBatch"];
        }
        mysql_query("INSERT INTO ActivityNight(NewFlag, CreateUserID, Batch) VALUES ('1', '$currentUserID', '$maxBatch')");
        $activityNightID = mysql_insert_id();
        $isNew = true;
    }
}
if ($_POST["action"] == "update"){ 
    $activityNightID = $_POST["ActivityNightID"];
    $batch = $_POST["Batch"];
    ...
    $quota = $_POST["Quota"];
    $vehicleOccupancy = $_POST["VehicleOccupancy"];
    $waitingNumber = $_POST["WaitingNumber"];
    $cost = $_POST["Cost"];
    $status = $_POST["Status"];
    $visibility = $_POST["Visibility"];
    $content = $_POST["ckeditor"];
    $sql_update = "UPDATE ActivityNight SET
                    Batch = '$batch',
    ...
    $sql_update .= "WHERE ActivityNightID = '$activityNightID'";
    //echo $sql_update;
    mysql_query($sql_update);
    #picture upload
    $targetFolder = "/home/metroasis/public_html/images/activityNight/";
    if (!file_exists($targetFolder)) {
        @mkdir($targetFolder);
    }
    if ($activityNightID != ""){
        $targetFolder = "/home/metroasis/public_html/images/activityNight/$activityNightID";
        if (!file_exists($targetFolder)) {
            @mkdir($targetFolder);
        }
    }
    $target_dir = "/home/metroasis/public_html/images/activityNight/$activityNightID/";
    $ImagefilePath = "/images/activityNight/$activityNightID/";
    $Message ="";
    if (isset($_FILES['imgFile1']) && $_FILES['imgFile1']['size'] > 0) {
        if (move_uploaded_file($_FILES['imgFile1']['tmp_name'], $target_dir . $_FILES['imgFile1']['name'])) {
            ...
            if ($ImageID1 != ""){
                ...
                $insert = "INSERT INTO ImagesFiles (ForeignID,ImageFunction,ImageType,ImagePath,ImageFileName) VALUES ('$activityNightID','activityNight','promotePicture','$ImagefilePath','". $_FILES['imgFile1']['name'] ."')";
            }                    
            //echo $insert;
            mysql_query($insert);
        }
    }else{
        $rec_image = mysql_query("SELECT COUNT(*) AS COUNT FROM ImagesFiles WHERE ForeignID = '$activityNightID'");
        $count_image = 0;
        if($rec_image){
    ...
    if ($Message != ""){
        ...
        echo "<script>parent.location.href='activity_night.php'; </script>";
    }
}
...
$rec_activity = mysql_query("SELECT *, (SELECT CONCAT(ImagePath, ImageFileName) FROM ImagesFiles WHERE ImageFunction = 'activityNight' AND ImageType = 'promotePicture' AND ForeignID = A.ActivityNightID LIMIT 1) AS ImageFile, (SELECT ImageID FROM ImagesFiles WHERE ImageFunction = 'activityNight' AND ImageType = 'promotePicture' AND ForeignID = A.ActivityNightID LIMIT 1) AS ImageID FROM ActivityNight A WHERE ActivityNightID = '$activityNightID'");
...
<head>
    ...
    <script type="text/javascript">
        function pageInitial() {
            $(document).find(".orange_18_de5106").html("❖ 熊鷹之夜");
            CKEDITOR.replace("ckeditor",{height:500,
                filebrowserUploadUrl: 'upload.php?funcId=activityNight&keyId=<? echo $activityNightID ?>&type=Files',
                on: {
        ...
        function beforeSubmit(){
            showLoading();
            var textFields = ["ActivityName","StartDate","EndDate","Campsite","Location","Quota","VehicleOccupancy","WaitingNumber","Cost"];
            var textNames = ["活動名稱","活動日期-起","活動日期-迄","營地","活動地點","活動車數","每車人數","可候補車數","營地費用"];
        ...
        <div style="overflow: hidden;">
            ...
            <div id="divWork" style="float: left; width: 90%">
                <div style="padding-left: 5px;">
                    <form method="post" action="activity_night.php">
                        <div style="height:10px;"></div>
                        <input type="submit" id="ibGo2List"  value="回列表"/>
                    </form>
                </div>
                <div class="divWorkArea" style="height:auto; margin-bottom: 100px;">
                	<form name="form1" method="post" action="activityDetail_night.php" id="form1" enctype="multipart/form-data">
                		<div id="UpdatePanel1" style="width:100%">
                			<div class="divDetailTopBar" style="width:100%">
                                <div id="ToolBar">
                                    <div style="float: left;">
                                        <? if($isNew){ ?>
                                            <span id="LabMessage" class="labMessage"><?echo "&nbsp;&nbsp;【". "新增 - 熊鷹之夜" ."】" ?></span>
                                        <? }else{ ?>
                                            <span id="LabMessage" class="labMessage"><?echo "&nbsp;&nbsp;【". "修改 - 熊鷹之夜" ."】" ?></span>
                                        <? } ?>
                                    </div>
                                </div>
                            </div>
                            <div id="divDetailBody" class="divDetailBody" style="padding-top: 0px;">
                            	<table class="TableLine" border="1px" cellpadding="5px" width="100%" bordercolor="#BABAD2">
                                    <tr height="45">
                                        ...
                                        <td width="18%" align="left" bgcolor="#ffffff" style="padding-left:10px;">
                                            <span class="gray_12">1-<?echo $row["Batch"] ?></span>
                                        ...
                                        <td width="36%" colspan="7" align="left" bgcolor="#ffffff">
                                            <table class="RadioButtonList" border="0">
                                                <tr>
                                                    <td>
                                    ...
                                    <tr height="45">
                                        ...
                                        <td align="left" bgcolor="#ffffff">
                                            <input type="date" name="StartDate" id="StartDate" class="TextBox" style="width: 120px;"  value="<?echo $row["StartDate"]?>"/>
                                            <span> ~ </span>
                                            <input type="date" name="EndDate"   id="EndDate"   class="TextBox" style="width: 120px;"  value="<?echo $row["EndDate"]?>"/>
                                        </td>
                                        <td align="center" bgcolor="#e5e5e5">
                                            <span id="Label20" class="DetailLabel"><span class="Mandatory">* 活動地點</span></span>
                                        </td>
                                        <td colspan="3" align="left" bgcolor="#ffffff">
                                            <input type="text" name="Location" id="Location"  class="TextBox" style="width: 95%;" value="<?echo $row["Location"] ?>"/>
                                        </td>
                                    </tr>
                                    <tr height="45">
                                        <td align="center" bgcolor="#e5e5e5">
                                            <span id="Label20" class="DetailLabel"><span class="Mandatory">* 營地</span></span>　　
                                        </td>
                                        <td align="left" bgcolor="#ffffff">
                                            <input type="text" name="Campsite" id="Campsite"  class="TextBox" style="width: 95%;" value="<?echo $row["Campsite"] ?>"/>
                                        </td>
                                        <td align="center" bgcolor="#e5e5e5">
                                        ...
                                        <td align="left" bgcolor="#ffffff">
                                        	...
                                        	<span class="b_12">輛</span>
                                        </td>
                                        <td align="center" bgcolor="#e5e5e5">
                                            <span id="Label20" class="DetailLabel"><span class="Mandatory">* 候補車數</span></span>
                                        </td>
                                        <td width="17%" align="left" bgcolor="#ffffff">
                                            <input type="number" name="WaitingNumber" id="WaitingNumber" class="TextBox" style="width:50px;" value="<?echo $row["WaitingNumber"] ?>" min="0" oninput="this.value=this.value.replace(/[^0-9]/g,'');"/>
                                        	<span class="b_12">輛</span>
                                        </td>
                                        <td width="8%" align="center" bgcolor="#e5e5e5">
                                            <span id="Label20" class="DetailLabel"><span class="Mandatory">* 每車人數</span></span>
                                        </td>
                                        <td width="17%" align="left" bgcolor="#ffffff">
                                            <input type="number" name="VehicleOccupancy" id="VehicleOccupancy"  class="TextBox" style="width:50px;" value="<?echo $row["VehicleOccupancy"] ?>" min="1" oninput="this.value=this.value.replace(/[^0-9]/g,'');"/>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="9" align="left" bgcolor="#ffffff" valign="top">
                                ...
                                <div style="width:100%; text-align:center">
                                    ...
                                    <input type="hidden" name="ActivityNightID" value="<? echo $activityNightID ?>"/>
