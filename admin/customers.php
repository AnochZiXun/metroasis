...
if(trim($eMail) != ""){
    $query_customer_sql = $eMail ? $query_customer_sql.' and EMail like "%'.trim($eMail).'%" ' : $query_customer_sql;
}
if(trim($identityNo) != ""){
                    	...
                    	<div id="divGridview">
							...
							<form action="" method="POST" enctype="multipart/form-data" id="startSearch" name="startSearch">
                                ...
                                <div style="float: left; padding-right:10px; padding-top: 3px;">
                                    <input type="text" class="TextBox" name="EMail" style="width:120px" value="<? echo $eMail ?>">
                                </div>
								<div style="float: left; height: 25px; padding-top: 5px; font-size:13px">
									&nbsp;&nbsp;身分證字號：
								</div>
                            ...
                            <div>
                                    <table class="GridView" cellspacing="0" cellpadding="5" rules="all" border="1" id="gvGridView"
										...
										<tr>
											...
											<td align="center">
												<input id="btnEdit" name="btnEdit" class="detail"  type="button" value="明細查詢" onclick="setdetail('<?echo $row["CustomerID"]?>')" />
                                                <!--
                                                &nbsp;&nbsp;&nbsp;&nbsp;
                            ...
                            <div class="GridViewFooter" align="center">
                                <table class="TableNoLine">
                                    <tr>
                                        ...
                                        <td>
											<a href="customers.php?page=1&IdentityNo=<?echo $identityNo?>&ChineseName=<?echo $chineseName?>&Mobile=<?echo $mobile?>&EMail=<?echo $eMail?>">最前頁｜</a>
                                        </td>
                                        <?	
											$prePage = $page-1;
											if ($prePage < 1) {
										...
										?>
										<td>
											<a href="customers.php?page=<?echo $prePage?>&IdentityNo=<?echo $identityNo?>&ChineseName=<?echo $chineseName?>&Mobile=<?echo $mobile?>&EMail=<?echo $eMail?>">上頁｜</a>
                                        </td>
										<?	
											$nextPage = $page+1;
										...
										?>
                                        <td>
											<a href="customers.php?page=<?echo $nextPage?>&IdentityNo=<?echo $identityNo?>&ChineseName=<?echo $chineseName?>&Mobile=<?echo $mobile?>&EMail=<?echo $eMail?>">下頁｜</a>
                                        </td>
                                        <td>
											<a href="customers.php?page=<?echo $total_pages?>&IdentityNo=<?echo $identityNo?>&ChineseName=<?echo $chineseName?>&Mobile=<?echo $mobile?>&EMail=<?echo $eMail?>">最後頁</a>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <select onChange="location = this.options[this.selectedIndex].value;">
                                              <?php
                                                ...
                                                {
                                              ?>
                                                    <option value="<?php $_SERVER['PHP_SELF']; ?>?page=<?php echo $i; ?>&IdentityNo=<?echo $identityNo?>&ChineseName=<?echo $chineseName?>&Mobile=<?echo $mobile?>&EMail=<?echo $eMail?>" <?php if($page==$i){echo "selected";} ?>>第 <?php echo $i; ?> 頁</option>
                                              <?php
                                                }
                                              ?>
                                            </select>
