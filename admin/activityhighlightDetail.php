...
if (isset($_GET["activityhighlightID"])) {
	$activityhighlightID = $_GET["activityhighlightID"];
}
if (isset($_POST["activityhighlightID"])) {
	$activityhighlightID = $_POST["activityhighlightID"];
}
$mode="0";
$modeDesc="修改";
$isNew = false;
if ($activityhighlightID == ""){
	$isNew = true;
	$insert_sql = "INSERT INTO Activityhighlight(NewFlag, CreateUserID) VALUES(1, '$currentUserID')";
	//echo $insert_sql;
	mysql_query($insert_sql);
	$activityhighlightID = mysql_insert_id();
	$modeDesc = "新增活動花絮";
	$mode = "1";
}else{
	$rec_newFlag = mysql_query("SELECT NewFlag FROM Activityhighlight WHERE ActivityhighlightID = '$activityhighlightID'");
	if($rec_newFlag){
...
if ($_POST["action"] == "update"){ 
	...
	$image = empty($_FILES['imgFile1']['name']) ? NULL : $_FILES['imgFile1']['name'];
	$update = "UPDATE Activityhighlight SET 
				Category = '$category', StartDate=$startDate,EndDate=$endDate,State=$state,
				ShortTitle='$shortTitle',
				ShortContent='$shortContent',ActivityDate='$activityDate',
				Title='$title',Content='$content', ";
	if($image != NULL){
		$update .= "Image='$image',";
	}
	$NewFlag = getFieldData($activityhighlightID,'Activityhighlight','ActivityhighlightID','NewFlag');
	switch($NewFlag){
    	case "0":
	...
	$update .= "NewFlag=0 WHERE ActivityhighlightID='$activityhighlightID'";
	//echo $update;
	mysql_query($update);
	//picture upload
    $targetFolder = "/home/metroasis/public_html/images/activityhighlight/";
    if (!file_exists($targetFolder)) {
    	@mkdir($targetFolder);
    }
    if ($activityhighlightID != ""){
    	$targetFolder = "/home/metroasis/public_html/images/activityhighlight/$activityhighlightID";
    	if (!file_exists($targetFolder)) {
    		@mkdir($targetFolder);
    	}
    }
    $target_dir = "/home/metroasis/public_html/images/activityhighlight/$activityhighlightID/";
	$ImagefilePath = "/images/activityhighlight/$activityhighlightID/";
	$Message ="";
	//500 * 333
		...
		if ($src_w==$intWidth && $src_h==$intHight){
			if (move_uploaded_file($_FILES['imgFile1']['tmp_name'], $target_dir . $_FILES['imgFile1']['name'])) {
				...
				if ($ImageID1 != ""){
					...
					$insert = "INSERT INTO ImagesFiles (ForeignID,ImageFunction,ImageType,ImagePath,ImageFileName) VALUES ('$activityhighlightID','activityhighlight','detail','$ImagefilePath','". $_FILES['imgFile1']['name'] ."')";
				}                    
				//echo $insert;
	            mysql_query($insert);
	        }
        } else {
	    	$Message = "「活動花絮 圖片」應為 (500 * 333)";
	    }
	}
	if ($Message != ""){
		echo "<script>alert('". $Message ."');</script>";
		mysql_query("INSERT INTO Activityhighlight (`Title`, `ShortTitle`, `Content`, `ShortContent`, `StartDate`, `EndDate`, `ActivityDate`, `State`, `Category`, `Image`, `IsTop`, `NewFlag`, CreateUserID, CreateDate, UpdateUserID, UpdateDate) SELECT `Title`, `ShortTitle`, `Content`, `ShortContent`, `StartDate`, `EndDate`, `ActivityDate`, `State`, `Category`, `Image`, `IsTop`, 1 AS NewFlag, CreateUserID, CreateDate, UpdateUserID, UpdateDate FROM Activityhighlight WHERE ActivityhighlightID = '$activityhighlightID' ");
		$activityhighlightId4delete = $activityhighlightID;
		$activityhighlightID = mysql_insert_id();
		mysql_query("DELETE FROM Activityhighlight WHERE ActivityhighlightID = '$activityhighlightId4delete'");
	}else{
		echo "<script>parent.location.href='activityhighlight.php'; </script>";
	}	
}
$query = "SELECT N.*,S.UserName AS createUserName, S2.UserName AS updateUserName, ";
$query .= "(SELECT concat(ImagePath,ImageFileName) as ImageFile FROM ImagesFiles WHERE ImageFunction = 'activityhighlight' AND ImageType ='detail' AND ForeignID ='$activityhighlightID' LIMIT 1) AS ImageFile, ";
$query .= "(SELECT ImageID FROM ImagesFiles WHERE ImageFunction = 'activityhighlight' AND ImageType ='detail' AND ForeignID ='$activityhighlightID' LIMIT 1) AS ImageID ";
$query .= "FROM Activityhighlight N ";
$query .= "LEFT JOIN SystemUsers S ON N.CreateUserID = S.UserID ";
$query .= "LEFT JOIN SystemUsers S2 ON N.UpdateUserID = S2.UserID ";
$query .= "WHERE N.ActivityhighlightID = '$activityhighlightID' ";
//echo $query;
$RecActivityhighlight = mysql_query($query);
$row=mysql_fetch_assoc($RecActivityhighlight);
?>
...
<head>
    ...
    <script type="text/javascript" charset="UTF-8">
		...
		function beforeSubmit(){
        	showLoading();
        	var textFields = ["ActivityDate","StartDate","ShortTitle","ShortContent","Title"];
        	var textNames = ["活動日期","上架日期","列表標題","短文","內頁標題"];
...
<body>
    <form name="form1" method="post" action="activityhighlightDetail.php" id="form1" enctype="multipart/form-data">
    <div>
            ...
            <div id="divDetailBody" class="divDetailBody" style="padding-top: 0px;">
                <table class="TableLine" border="1px" cellpadding="5px" width="100%" bordercolor="#BABAD2">
                    <tr>
                        ...
                        <td align="center" style="width: 12%" bgcolor="#ffffff">
						...
						<td align="center" style="width: 12%" bgcolor="#ffffff">
							<?
								$findCategory = "SELECT * FROM RefCommon WHERE Type = 'activityhighlight' AND TypeCode <> 0 ORDER BY SortNo";
                        ...
                        <td align="center" bgcolor="#ffffff" style="width: 12%">
					...
					<tr>
                      <td  align="center" bgcolor="#e5e5e5">
                            <span id="labMemberCode" class="DetailLabel"><span class="Mandatory">* 列表標題</span></span>
                        </td>
                  ...
                  <td colspan="5" align="center" bgcolor="#ffffff">
                            <input name="ShortContent" type="text" id="ShortContent" class="TextBox" style="width: 98%;" value="<?echo $row["ShortContent"]?>"/>
                        </td>
					</tr>	
					<tr>
                        <td  align="center" bgcolor="#e5e5e5">
                            <span id="labMemberCode" class="DetailLabel"><span class="Mandatory">* 內頁標題</span></span>
                        </td>
                        <td colspan="9" align="center" bgcolor="#ffffff">
                            <input name="Title" type="text" id="Title" class="TextBox" style="width: 99%;" value="<?echo $row["Title"]?>"/>
                        </td>
					</tr>
                    <tr>
                ...
                <div style="width:100%; text-align:center">
            		...
            		<input type="hidden" name="activityhighlightID" value="<? echo $activityhighlightID?>"/>
